...
from ...unit.infrastructure.test_infrastructure import (
...
def test_branch_topology_full_workflow():
    ...
    test_config = TestProjectInventory.create_isolated_project("test_branch_topology_full_workflow")
    
    # 2. Use completely isolated temp directory  
    with isolated_temporary_directory("test_branch_topology_full_workflow") as test_dir:
        try:
...
"""
            ...
            print(f"Qdrant host: {loaded_config.qdrant.host}")
            if hasattr(loaded_config, "project_ports") and loaded_config.project_ports:
                print(f"Dynamic ports - Qdrant: {loaded_config.project_ports.qdrant_port}")
...
"""
            ...
            # Success cleanup
            service_manager.cleanup_qdrant_collections_via_api(f"test_{test_config.name}")
            
        except Exception as e:
            # Failure artifact preservation  
            artifact_manager = FailureArtifactManager(test_config.name)
            artifact_manager.preserve_on_failure(test_dir, e)
            raise
        finally:
            try:
