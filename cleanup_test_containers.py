...
def cleanup_dual_engine_containers():
    ...
    for engine in ["podman", "docker"]:
        for container_name in ["code-indexer-ollama", "code-indexer-qdrant"]:
            try:
                subprocess.run([engine, "stop", container_name], capture_output=True, timeout=30)
                subprocess.run([engine, "rm", container_name], capture_output=True, timeout=30)
        ...
        try:
            subprocess.run([engine, "network", "rm", "code-indexer-global"], capture_output=True, timeout=30)
            print(f"✅ Cleaned up legacy network code-indexer-global")
        except Exception:
            pass
