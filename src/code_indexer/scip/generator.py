...
class SCIPGenerator:
        ...
        for project in projects:
            ...
            if indexer_result.is_success():
                ...
                if progress_callback:
                    progress_callback(
                        project,
                        f"✗ {project.relative_path} (failed)"
                    )
    ...
    def rebuild_projects(
        ...
        if failed_only:
            projects_to_rebuild = [
                path for path, status in current_status.projects.items()
        ...
        for project_path in projects_to_rebuild:
            ...
            indexer_result = indexer.generate(
                project_dir,
                output_dir,
                project_status.build_system
            )
            
            # Create updated status
            new_status = ProjectStatus(
                status=OverallStatus.SUCCESS if indexer_result.is_success() else OverallStatus.FAILED,
                language=project_status.language,
                build_system=project_status.build_system,
                timestamp=datetime.now().isoformat(),
                duration_seconds=indexer_result.duration_seconds,
                output_file=str(indexer_result.output_file) if indexer_result.output_file else None,
        ...
        successful = sum(1 for s in current_status.projects.values() if s.status == OverallStatus.SUCCESS)
        failed = sum(1 for s in current_status.projects.values() if s.status == OverallStatus.FAILED)
