...
class FileFinder:
    ...
    def find_files(self) -> Iterator[Path]:
        ...
        if not self.config.codebase_dir.exists():
            raise ValueError(f"Codebase directory does not exist: {self.config.codebase_dir}")
        
        if not self.config.codebase_dir.is_dir():
            raise ValueError(f"Codebase path is not a directory: {self.config.codebase_dir}")
