...
class ChangeDetector:
    ...
    def get_local_ref(self) -> str:
        ...
        if result.returncode != 0:
            logger.error(f"Git rev-parse HEAD failed: {result.stderr}")
            raise subprocess.CalledProcessError(
                result.returncode,
                result.args,
                result.stdout,
        ...
        logger.debug(f"Local ref: {local_ref}")
        return local_ref
    ...
    def get_remote_ref(self) -> str:
        ...
        if result.returncode != 0:
            logger.error(f"Git rev-parse origin/{self.branch} failed: {result.stderr}")
            raise subprocess.CalledProcessError(
                result.returncode,
                result.args,
                result.stdout,
        ...
        logger.debug(f"Remote ref: {remote_ref}")
        return remote_ref
    ...
    def has_changes(self) -> bool:
        ...
        if has_changes:
            logger.info(f"Changes detected: {local_ref} -> {remote_ref}")
        else:
