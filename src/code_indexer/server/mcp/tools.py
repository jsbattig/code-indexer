...
TOOL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    "search_code": {
        "name": "search_code",
        "description": "Search code using semantic search, FTS, or hybrid mode. ALIAS GLOSSARY: 'repository_alias' is what you search with - global repos end in '-global' (e.g., 'backend-global'), activated repos use custom aliases. Use list_global_repos to discover available global repo aliases. DISCOVERY WORKFLOW: For exploring unfamiliar codebases, start with 'cidx-meta-global' which contains .md descriptions of all registered repositories. You can either: (1) search_code('API endpoints', repository_alias='cidx-meta-global') for semantic discovery, or (2) browse_directory(repository_alias='cidx-meta-global') to see all repository description files. Then query specific repositories for detailed code. Example: search_code('API endpoints', repository_alias='cidx-meta-global') finds repos with APIs, then search_code('user authentication endpoint', repository_alias='backend-global') for implementation details. TROUBLESHOOTING: (1) Empty results? Check: repository_alias is correct and ends in '-global' for global repos, repo is indexed (use global_repo_status), try broader query terms. (2) 'Repository not found'? List available repos with list_global_repos first. (3) Slow queries? Reduce limit, use path_filter to narrow scope. (4) cidx-meta-global empty? Server may need restart to complete bootstrap indexing.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query_text": {
