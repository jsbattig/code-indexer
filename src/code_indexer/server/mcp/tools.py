...
""",
    ...
    "get_file_content": {
        ...
        "description": "TL;DR: Read file content from repository with metadata (size, language, modified_at). Returns MCP-compliant content blocks (text format). QUICK START: get_file_content('backend-global', 'src/auth.py') reads file. USE CASES: (1) Read source code after search_code identifies relevant files, (2) Inspect configuration files, (3) Review file content before editing. OUTPUT FORMAT: Returns array of content blocks following MCP specification - each block has type='text' and text=file_content. Metadata includes file size, detected language, modification timestamp. WHEN TO USE: After identifying target file via search_code, browse_directory, or list_files. WHEN NOT TO USE: (1) Need file listing -> use list_files or browse_directory, (2) Need to search content -> use search_code or regex_search first, (3) Need directory structure -> use directory_tree. TROUBLESHOOTING: File not found? Verify file_path with list_files or browse_directory. Permission denied? Check repository is activated and accessible. RELATED TOOLS: list_files (find files), search_code (search content), edit_file (modify content), browse_directory (list with metadata).",
        "inputSchema": {
            "type": "object",
            "properties": {
                "repository_alias": {
