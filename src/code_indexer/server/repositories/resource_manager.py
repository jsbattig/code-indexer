...
class ResourceTracker:
    ...
    def track_database_connection(self, connection: Any, connection_name: str) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Tracking database connection: {connection_name}", extra={"correlation_id": get_correlation_id()})

    def track_temp_file(self, temp_path: Path) -> None:
        """Track a temporary file for cleanup."""
        with self._lock:
            self.temp_files.add(temp_path)
            logger.debug(f"Tracking temp file: {temp_path}", extra={"correlation_id": get_correlation_id()})
    ...
    def track_background_task(self, task: asyncio.Task, task_name: str) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Tracking background task: {task_name}", extra={"correlation_id": get_correlation_id()})
        ...
        with self._lock:
            for file_handle in list(self.file_handles):
                try:
                    ...
                    logger.warning(error_msg, extra={"correlation_id": get_correlation_id()})
            ...
            for conn_name, connection in list(self.database_connections.items()):
                try:
                    if hasattr(connection, "close"):
                        connection.close()
                        logger.debug(f"Closed database connection: {conn_name}", extra={"correlation_id": get_correlation_id()})
                except Exception as e:
                ...
                try:
                    if temp_path.exists():
                        if temp_path.is_dir():
                            ...
                            logger.debug(f"Removed temp directory: {temp_path}", extra={"correlation_id": get_correlation_id()})
                        else:
                            temp_path.unlink()
                            logger.debug(f"Removed temp file: {temp_path}", extra={"correlation_id": get_correlation_id()})
                except Exception as e:
        ...
        for task_name, task in tasks_to_cancel:
            try:
                if not task.done():
                    ...
                    logger.debug(f"Cancelled background task: {task_name}", extra={"correlation_id": get_correlation_id()})
            except Exception as e:
...
class MemoryMonitor:
    ...
    def __init__(self, leak_threshold_mb: float = 50.0):
        ...
        logger.info(f"Memory baseline captured: {self.baseline_memory_mb:.1f}MB", extra={"correlation_id": get_correlation_id()})
    ...
    def _capture_current_memory(self) -> float:
        ...
        try:
            ...
            logger.warning(f"Failed to capture memory usage: {e}", extra={"correlation_id": get_correlation_id()})
            return 0.0
    ...
    def get_process_memory_percent(self) -> float:
        ...
        try:
            ...
            logger.warning(f"Failed to get memory percentage: {e}", extra={"correlation_id": get_correlation_id()})
            return 0.0
    ...
    def force_garbage_collection(self) -> int:
        ...
        try:
            ...
            logger.debug(f"Garbage collection freed {collected_objects} objects", extra={"correlation_id": get_correlation_id()})
            return collected_objects
        except Exception as e:
            logger.warning(f"Garbage collection failed: {e}", extra={"correlation_id": get_correlation_id()})
            return 0
...
class GracefulShutdownHandler:
    ...
    def register_handlers(self) -> None:
        ...
        logger.info("Graceful shutdown handlers registered for SIGTERM and SIGINT", extra={"correlation_id": get_correlation_id()})

    def register_cleanup_callback(self, callback: Callable[[], None]) -> None:
        """Register synchronous cleanup callback."""
        with self._lock:
            self.cleanup_callbacks.append(callback)
            logger.debug(f"Registered cleanup callback: {callback.__name__}", extra={"correlation_id": get_correlation_id()})
    ...
    def register_async_cleanup_callback(self, callback: Callable[[], Any]) -> None:
        ...
        with self._lock:
            ...
            logger.debug(f"Registered async cleanup callback: {callback.__name__}", extra={"correlation_id": get_correlation_id()})

    def _signal_handler(self, signum: int, frame) -> None:
        """Handle shutdown signals and execute cleanup."""
        signal_name = signal.Signals(signum).name
        logger.info(f"Received {signal_name} signal, initiating graceful shutdown", extra={"correlation_id": get_correlation_id()})

        with self._lock:
            if self._cleanup_executed:
                logger.info("Cleanup already executed, ignoring additional signals", extra={"correlation_id": get_correlation_id()})
                return
    ...
    def _execute_cleanup_with_timeout(self) -> None:
        ...
        for callback in self.cleanup_callbacks:
            ...
            try:
                ...
                logger.error(f"Cleanup callback {callback.__name__} failed: {e}", extra={"correlation_id": get_correlation_id()})
    ...
    def _execute_async_cleanup(self, timeout: float) -> None:
        ...
        try:
            ...
            try:
                ...
                def schedule_async_cleanup():
                    try:
                        ...
                        logger.error(f"Failed to schedule async cleanup: {e}", extra={"correlation_id": get_correlation_id()})
                ...
                async def run_async_cleanup():
                    ...
                    if tasks:
                        try:
                            ...
                            logger.warning("Async cleanup timeout exceeded", extra={"correlation_id": get_correlation_id()})
                            for task in tasks:
                                if not task.done():
                                    task.cancel()
            ...
            logger.error(f"Async cleanup execution failed: {e}", extra={"correlation_id": get_correlation_id()})
...
class ResourceManager:
    ...
    def __init__(
        ...
        logger.debug("ResourceManager initialized", extra={"correlation_id": get_correlation_id()})

    async def __aenter__(self):
        """Async context manager entry."""
        logger.debug("ResourceManager context entered", extra={"correlation_id": get_correlation_id()})
        return self

    async def __aexit__(self, exc_type, exc_val, exc_tb):
        """Async context manager exit with comprehensive cleanup."""
        logger.debug("ResourceManager context exiting, starting cleanup", extra={"correlation_id": get_correlation_id()})

        try:
            await self.cleanup_all()
        except Exception as e:
            logger.error(f"ResourceManager cleanup failed: {e}", extra={"correlation_id": get_correlation_id()})
            # Don't suppress exceptions from the context

        logger.debug("ResourceManager context exited", extra={"correlation_id": get_correlation_id()})
    ...
    async def cleanup_all(self) -> None:
        ...
        logger.info("Starting comprehensive resource cleanup", extra={"correlation_id": get_correlation_id()})
        ...
        if self.memory_monitor:
            try:
                ...
                logger.debug(f"Garbage collection freed {collected} objects", extra={"correlation_id": get_correlation_id()})
            except Exception as e:
        ...
        if total_errors == 0:
            logger.info("Resource cleanup completed successfully", extra={"correlation_id": get_correlation_id()})
        else:
            logger.warning(f"Resource cleanup completed with {total_errors} errors", extra={"correlation_id": get_correlation_id()})
            for error in cleanup_errors:
                logger.warning(f"  - {error}", extra={"correlation_id": get_correlation_id()})
            ...
            try:
                ...
                if leak_warnings:
                    for warning in leak_warnings:
                        logger.warning(f"Memory leak detected after cleanup: {warning}", extra={"correlation_id": get_correlation_id()})
            except Exception as e:
                logger.error(f"Memory leak check failed: {e}", extra={"correlation_id": get_correlation_id()})
...
def setup_graceful_shutdown(
    ...
    def cleanup_resources():
        try:
            ...
            logger.error(f"Resource cleanup during shutdown failed: {e}", extra={"correlation_id": get_correlation_id()})

    shutdown_handler.register_cleanup_callback(cleanup_resources)
    shutdown_handler.register_handlers()

    logger.info("Graceful shutdown configured with resource cleanup", extra={"correlation_id": get_correlation_id()})
