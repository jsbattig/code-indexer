...
class ProgressCallbackIntegrator:
    ...
    def __init__(
        ...
        logger.debug(
            f"ProgressCallbackIntegrator initialized for job {job_id}",
    ...
    def _extract_indexing_metrics(self, info: str) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                f"Failed to extract metrics from '{info}': {e}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def _ensure_phase_started(self, phase: str):
        ...
        try:
            ...
            logger.debug(
                f"Could not start phase {phase}: {e}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def _complete_phase_if_needed(self, phase: str):
        ...
        try:
            ...
            if (
                ...
                logger.debug(
                    f"Auto-completed phase {phase} at 100% progress",
                    extra={"correlation_id": get_correlation_id()},
                )

        except Exception as e:
            logger.debug(
                f"Could not complete phase {phase}: {e}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def _handle_phase_transition(self, new_phase: str):
        ...
        try:
            ...
            logger.debug(
                f"Could not handle phase transition to {new_phase}: {e}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def _ensure_legacy_phase_started(self, phase: str):
        ...
        try:
            ...
            logger.debug(
                f"Could not start legacy phase {phase}: {e}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def start_phase(self, phase_name: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Manually started phase '{phase_name}' for job {self.job_id}",
                extra={"correlation_id": get_correlation_id()},
            )
    ...
    def complete_phase(
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    f"Legacy phase completion failed for {phase_name}: {e}",
                    extra={"correlation_id": get_correlation_id()},
                )
