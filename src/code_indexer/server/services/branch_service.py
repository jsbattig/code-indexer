...
class BranchService:
    ...
    def list_branches(self, include_remote: bool = False) -> List[BranchInfo]:
        ...
        try:
            ...
            logger.error("Git operation failed listing branches", exc_info=True)
            raise  # Preserve original exception
        except Exception as e:
            logger.error("Unexpected error listing branches", exc_info=True)
    ...
    def close(self):
        ...
        if not self._closed and hasattr(self, "repo"):
            try:
                ...
                logger.warning(f"Error closing git repository: {e}")
            finally:
                self._closed = True
