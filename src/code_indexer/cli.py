...
@cli.command()
@click.option('--codebase-dir', '-d', type=click.Path(exists=True), help='Directory to index')
...
def init(ctx, codebase_dir: Optional[str], force: bool):
    ...
    if config_manager.config_path.exists() and not force:
        console.print(f"❌ Configuration already exists at {config_manager.config_path}")
    ...
    try:
        ...
        console.print(f"🔧 Run 'code-indexer setup' to start services")
...
def setup(ctx, model: Optional[str], force_recreate: bool, quiet: bool):
    ...
    try:
        ...
        if not docker_manager.is_docker_available():
            setup_console.print("❌ Docker is not available. Please install Docker first.", style="red")
            sys.exit(1)
        
        if not docker_manager.is_compose_available():
            setup_console.print("❌ Docker Compose is not available. Please install Docker Compose first.", style="red")
            sys.exit(1)
        ...
        if not ollama_client.model_exists(config.ollama.model):
            if not ollama_client.pull_model(config.ollama.model):
                setup_console.print(f"❌ Failed to pull model {config.ollama.model}", style="red")
                sys.exit(1)
...
def index(ctx, clear: bool, batch_size: int):
    ...
    try:
        ...
        # Health checks
        if not ollama_client.health_check():
            console.print("❌ Ollama service not available. Run 'setup' first.", style="red")
            sys.exit(1)
        
        if not qdrant_client.health_check():
            console.print("❌ Qdrant service not available. Run 'setup' first.", style="red")
            sys.exit(1)
                    ...
                    for chunk in chunks:
                        ...
                        if len(batch_points) >= batch_size:
                            if not qdrant_client.upsert_points(batch_points):
                                console.print(f"❌ Failed to index batch", style="red")
                            batch_points = []
                    
                except Exception as e:
                    if ctx.obj['verbose']:
                        console.print(f"❌ Failed to process {file_path}: {e}", style="red")
                    failed_files += 1
        ...
        console.print(f"✅ Indexing complete!", style="green")
...
@cli.command()
@click.option('--since', type=click.DateTime(), help='Update files modified since this time')
...
def update(ctx, since: Optional[str], batch_size: int):
    ...
    try:
        ...
        if not since:
            ...
            if not since:
                console.print("❌ No previous index found. Use 'code-indexer index' first.", style="red")
                sys.exit(1)
        ...
        if deleted_files:
            ...
            for deleted_file in deleted_files:
                try:
                    ...
                    if ctx.obj['verbose']:
                        console.print(f"❌ Failed to delete {deleted_file}: {e}", style="red")
                ...
                for file_path in modified_files:
                    try:
                        ...
                        for chunk in chunks:
                            ...
                            if len(batch_points) >= batch_size:
                                if not qdrant_client.upsert_points(batch_points):
                                    console.print(f"❌ Failed to update batch", style="red")
                                batch_points = []
        ...
        index_metadata = {
            "indexed_at": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
            "files_updated": len(modified_files) - failed_files if modified_files else 0,
        ...
        console.print(f"✅ Index update complete!", style="green")
        if modified_files:
    ...
    try:
        ...
        class CodeChangeHandler(FileSystemEventHandler):
            def on_modified(self, event):
                ...
                try:
                    relative_path = file_path.relative_to(config.codebase_dir)
        ...
        def process_changes():
            ...
            while True:
                ...
                console.print(f"📁 Processing {len(changes_to_process)} changed files...")
                ...
                if modified_files or deleted_files:
                    console.print(f"✅ Updated index: {len(modified_files)} modified, {len(deleted_files)} deleted", style="green")
...
@click.pass_context
def query(ctx, query: str, limit: int, language: Optional[str], path: Optional[str], min_score: Optional[float]):
    ...
    try:
        ...
        filter_conditions = {}
        if language:
            filter_conditions["must"] = [{"key": "language", "match": {"value": language}}]
        if path:
        ...
        for i, result in enumerate(results, 1):
            ...
            if content:
                ...
                if language and language != "unknown":
                    try:
                        syntax = Syntax(content[:500], language, theme="monokai", line_numbers=False)
...
def status(ctx):
    ...
    try:
        ...
        docker_status = "✅ Running" if service_status["status"] == "running" else "❌ Not Running"
        table.add_row("Docker Services", docker_status, f"{len(service_status['services'])} services")
        
        # Check Ollama
        ollama_client = OllamaClient(config.ollama)
        ollama_ok = ollama_client.health_check()
        ollama_status = "✅ Ready" if ollama_ok else "❌ Not Available"
        ollama_details = f"Model: {config.ollama.model}" if ollama_ok else "Service down"
    ...
    if install:
        ...
        for hook_name, hook_content in hooks.items():
            ...
            if hook_path.exists():
                ...
                console.print(f"📦 Backed up existing {hook_name} to {hook_name}.backup")
        ...
        for hook_name in hooks.keys():
            ...
            if hook_path.exists():
                ...
                if "Code Indexer:" in content:
                    ...
                    console.print(f"⚠️  {hook_name} exists but not from Code Indexer, skipping")
        
        console.print("✅ Git hooks removed successfully!")


@cli.command()
@click.option('--remove-data', '-d', is_flag=True, help='Remove all data and configuration')
@click.option('--quiet', '-q', is_flag=True, help='Suppress output')
@click.pass_context
