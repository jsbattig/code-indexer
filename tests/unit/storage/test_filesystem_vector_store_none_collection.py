...
class TestFilesystemVectorStoreNoneCollection:
    ...
    def test_upsert_points_with_none_collection_name_auto_resolves_single_collection(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            store = FilesystemVectorStore(
                base_path=base_path, project_root=project_root
            )
    ...
    def test_upsert_points_with_none_collection_auto_resolves_to_only_collection(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = store.upsert_points(collection_name=None, points=[test_point])
    ...
    def test_upsert_points_with_none_collection_raises_error_when_multiple_collections(
        self,
    ):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with pytest.raises(ValueError) as exc_info:
                store.upsert_points(collection_name=None, points=[test_point])

            error_message = str(exc_info.value)
            assert "collection_name" in error_message.lower()
            assert (
                "required" in error_message.lower()
                or "ambiguous" in error_message.lower()
            )
