...
class TestChangeTrackingInitialization:
    ...
    def test_begin_indexing_initializes_change_tracking(
        self, vector_store, test_collection
    ):
...
class TestUpsertPointsChangeTracking:
    ...
    def test_upsert_new_points_tracks_as_added(
        self, vector_store, test_collection, sample_points
    ):
    ...
    def test_upsert_existing_points_tracks_as_updated(
        self, vector_store, test_collection, sample_points
    ):
    ...
    def test_upsert_without_begin_indexing_no_tracking(
        self, vector_store, test_collection, sample_points
    ):
        ...
        assert (
            not hasattr(vector_store, "_indexing_session_changes")
            or test_collection not in vector_store._indexing_session_changes
        )
...
class TestDeletePointsChangeTracking:
    ...
    def test_delete_points_tracks_as_deleted(
        self, vector_store, test_collection, sample_points
    ):
...
class TestWatchModeParameter:
    ...
    def test_upsert_points_accepts_watch_mode_parameter(
        self, vector_store, test_collection, sample_points
    ):
        ...
        try:
            vector_store.upsert_points(
                test_collection, sample_points[:5], watch_mode=True
            )
    ...
    def test_watch_mode_triggers_immediate_hnsw_update(
        self, vector_store, test_collection, sample_points
    ):
        ...
        vector_store.upsert_points(test_collection, sample_points[:5], watch_mode=True)
...
class TestEndIndexingAutoDetection:
    ...
    def test_end_indexing_with_changes_triggers_incremental(
        self, vector_store, test_collection, sample_points
    ):
        ...
        vector_store.begin_indexing(test_collection)
        vector_store.upsert_points(
            test_collection, sample_points[5:10]
        )  # Add 5 more points
    ...
    def test_end_indexing_first_index_triggers_full_rebuild(
        self, vector_store, test_collection, sample_points
    ):
    ...
    def test_end_indexing_clears_session_changes(
        self, vector_store, test_collection, sample_points
    ):
...
class TestChangeTrackingMultipleSessions:
    ...
    def test_multiple_sessions_independent_tracking(
        self, vector_store, test_collection, sample_points
    ):
