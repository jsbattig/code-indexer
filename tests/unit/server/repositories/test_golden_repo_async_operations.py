...
class TestGoldenRepoAsyncOperations:
    ...
    def test_refresh_golden_repo_submits_background_job(self, manager_with_existing_repo):
        ...
        with patch("subprocess.run") as mock_run:
            ...
            call_args = manager_with_existing_repo.background_job_manager.submit_job.call_args
    ...
    def test_refresh_golden_repo_validates_before_job_submission(self, golden_repo_manager):
        """Test that refresh_golden_repo validates repo exists before submitting job."""
        with pytest.raises(GoldenRepoError, match="Golden repository 'nonexistent' not found"):
    ...
    def test_add_golden_repo_returns_job_id(self, golden_repo_manager):
        ...
        with patch.object(golden_repo_manager, "_validate_git_repository") as mock_validate:
    ...
    def test_add_golden_repo_validates_before_job_submission(self, golden_repo_manager):
        ...
        with patch.object(golden_repo_manager, "_validate_git_repository") as mock_validate:
            ...
            with pytest.raises(GoldenRepoError, match="Golden repository alias 'duplicate' already exists"):
                golden_repo_manager.add_golden_repo(
    ...
    def test_add_golden_repo_validates_max_repos_limit(self, golden_repo_manager):
        ...
        with patch.object(golden_repo_manager, "_validate_git_repository") as mock_validate:
            ...
            with pytest.raises(ResourceLimitError, match=f"Maximum of {GoldenRepoManager.MAX_GOLDEN_REPOS} golden repositories allowed"):
                golden_repo_manager.add_golden_repo(
    ...
    def test_remove_golden_repo_submits_background_job(self, manager_with_existing_repo):
    ...
    def test_remove_golden_repo_validates_before_job_submission(self, golden_repo_manager):
    ...
    def test_refresh_golden_repo_background_worker_callable(self, manager_with_existing_repo):
    ...
    def test_remove_golden_repo_background_worker_callable(self, manager_with_existing_repo):
    ...
    def test_remove_golden_repo_cleanup_failure_raises_exception(self, manager_with_existing_repo):
        ...
        with patch.object(manager_with_existing_repo, "_cleanup_repository_files") as mock_cleanup:
            ...
            from src.code_indexer.server.repositories.golden_repo_manager import GitOperationError
            with pytest.raises(GitOperationError, match="cleanup incomplete|Resource leak"):
                background_worker()

    def test_remove_golden_repo_cleanup_success_no_exception(self, manager_with_existing_repo):
    ...
    def test_add_golden_repo_passes_username_to_job_manager(self, golden_repo_manager):
        ...
        with patch.object(golden_repo_manager, "_validate_git_repository") as mock_validate:
            ...
            golden_repo_manager.background_job_manager.submit_job.assert_called_once()
            call_kwargs = golden_repo_manager.background_job_manager.submit_job.call_args[1]
            assert call_kwargs["submitter_username"] == "alice"
            assert call_kwargs["is_admin"] is True

    def test_remove_golden_repo_passes_username_to_job_manager(self, manager_with_existing_repo):
        """Verify actual username is passed to background job manager for removal."""
        manager_with_existing_repo.remove_golden_repo("test-repo", submitter_username="bob")

        # Verify submit_job was called with correct username
        call_kwargs = manager_with_existing_repo.background_job_manager.submit_job.call_args[1]
        assert call_kwargs["submitter_username"] == "bob"
        assert call_kwargs["is_admin"] is True

    def test_refresh_golden_repo_passes_username_to_job_manager(self, manager_with_existing_repo):
        """Verify actual username is passed to background job manager for refresh."""
        manager_with_existing_repo.refresh_golden_repo("test-repo", submitter_username="charlie")
    ...
    def test_remove_golden_repo_default_username_is_admin(self, manager_with_existing_repo):
        """Verify default username is 'admin' when not specified."""
        # Call without submitter_username parameter
        manager_with_existing_repo.remove_golden_repo("test-repo")
    ...
    def test_refresh_golden_repo_default_username_is_admin(self, manager_with_existing_repo):
        """Verify default username is 'admin' when not specified."""
        # Call without submitter_username parameter
        manager_with_existing_repo.refresh_golden_repo("test-repo")
