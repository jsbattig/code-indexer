...
@pytest.fixture
def scheduler(mock_golden_repos_dir, mock_config_source, mock_query_tracker, mock_cleanup_manager):
...
def test_concurrent_refreshes_different_repos_no_interference(scheduler, mock_git_pull_updater):
    """
    ...
    """
    with patch.object(scheduler.registry, 'get_global_repo') as mock_get_repo, \
         ...
         patch('code_indexer.global_repos.refresh_scheduler.GitPullUpdater') as mock_updater_class:

        # Setup mocks
        mock_get_repo.side_effect = lambda alias: {
            'alias_name': alias,
            'repo_url': 'https://github.com/user/repo1.git' if alias == 'repo1-global' else 'https://github.com/user/repo2.git',
        }
        ...
        thread1 = threading.Thread(target=scheduler.refresh_repo, args=('repo1-global',))
        thread2 = threading.Thread(target=scheduler.refresh_repo, args=('repo2-global',))
        ...
        assert len(refresh_events['repo1']) == 2, "Repo1 refresh should have started and ended"
        assert len(refresh_events['repo2']) == 2, "Repo2 refresh should have started and ended"

        # Verify concurrent execution (total time should be < sum of individual times)
        assert elapsed_time < 0.15, f"Refreshes should run concurrently (took {elapsed_time}s)"
        ...
        assert repo2_start < repo1_end, "Repo2 should start before repo1 completes (concurrent execution)"
...
def test_concurrent_refreshes_same_repo_serialized(scheduler, mock_git_pull_updater):
    ...
    with patch.object(scheduler.registry, 'get_global_repo') as mock_get_repo, \
        ...
        thread1 = threading.Thread(target=scheduler.refresh_repo, args=('test-repo-global',), name='refresh1')
        thread2 = threading.Thread(target=scheduler.refresh_repo, args=('test-repo-global',), name='refresh2')
        ...
        # Verify both refreshes completed
        assert len(refresh_order) == 4, f"Both refreshes should complete (got {len(refresh_order)} events)"

        # Verify serialization: second refresh should not start until first completes
        first_start = next(i for i, (event, thread) in enumerate(refresh_order) if event == 'start')
        first_end = next(i for i, (event, thread) in enumerate(refresh_order) if event == 'end')
        second_start = next(i for i, (event, thread) in enumerate(refresh_order) if event == 'start' and i > first_start)

        assert first_end < second_start, f"Second refresh should start after first completes (order: {refresh_order})"

        # Verify sequential execution (total time should be ~sum of individual times)
        assert elapsed_time >= 0.2, f"Refreshes should run sequentially (took {elapsed_time}s)"
...
def test_refresh_lock_released_on_timeout(scheduler, mock_git_pull_updater):
    ...
    with patch.object(scheduler.registry, 'get_global_repo') as mock_get_repo, \
        ...
        thread1 = threading.Thread(target=scheduler.refresh_repo, args=('test-repo-global',))
        thread1.start()
        ...
        thread2 = threading.Thread(target=scheduler.refresh_repo, args=('test-repo-global',))
        thread2.start()
        ...
        assert not thread2.is_alive(), "Thread2 should have completed or timed out gracefully"
...
def test_refresh_lock_prevents_duplicate_refresh(scheduler, mock_git_pull_updater):
    ...
    with patch.object(scheduler.registry, 'get_global_repo') as mock_get_repo, \
        ...
        assert create_count[0] == 3, f"All 3 refresh attempts should complete sequentially (got {create_count[0]})"
