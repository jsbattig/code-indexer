...
class TestTokenAwareBatching:
    ...
    def voyage_config(self):
        ...
        return VoyageAIConfig(
            batch_size=128, max_retries=2, retry_delay=1.0, exponential_backoff=True
        )
    ...
    def test_token_limit_respected_for_large_batches(self, voyage_client):
        ...
        mock_responses = [
            {"data": [{"embedding": [0.1, 0.2, 0.3]} for _ in range(len(batch))]}
            for batch in [large_texts[:35], large_texts[35:]]  # Split into 2 batches
        ]

        with patch.object(
            voyage_client, "_make_sync_request", side_effect=mock_responses
        ):
    ...
    def test_token_estimation_accuracy(self, voyage_client):
        ...
        test_cases = [
            ...
            (
                "this is a longer piece of text with many words",
                9,
            ),  # 10 words * 0.75 = 7.5 → 8 tokens
        ]

        for text, expected_min_tokens in test_cases:
            estimated = voyage_client._estimate_tokens(text)
            assert (
                estimated >= expected_min_tokens
            ), f"Text '{text}' should estimate at least {expected_min_tokens} tokens, got {estimated}"
    ...
    def test_single_batch_when_under_limits(self, voyage_client):
        ...
        mock_response = {"data": [{"embedding": [0.1, 0.2, 0.3]} for _ in range(10)]}

        with patch.object(
            voyage_client, "_make_sync_request", return_value=mock_response
        ) as mock_api:
    ...
    def test_multiple_batches_for_large_files(self, voyage_client):
        ...
        with patch.object(
            voyage_client, "_make_sync_request", side_effect=mock_api_call
        ):
            ...
            assert (
                call_count > 1
            ), f"Large batch should require multiple API calls, got {call_count}"
            assert (
                len(embeddings) == 200
            ), "All embeddings should be returned despite batching"
