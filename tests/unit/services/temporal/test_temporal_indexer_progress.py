...
class TestTemporalIndexerProgress:
    ...
    def test_parallel_processing_reports_real_progress(self, mock_diff_scanner_class, mock_vector_manager_class, mock_factory):
        ...
        assert first_call['total'] == 10, f"Expected total=10, got {first_call['total']}"
        ...
        for call in progress_calls:
            ...
            assert " - " in info.split("📝")[-1] if "📝" in info else False, f"Missing 'hash - file' format after 📝: {info}"
    ...
    def test_progress_shows_actual_filenames_not_test_py(self, mock_diff_scanner_class, mock_vector_manager_class, mock_factory):
        ...
        for info in infos:
            ...
            if "📝" in info:
                ...
                if len(parts) > 1:
                    ...
                    assert " - " in hash_file_part, f"Missing 'hash - file' format: {info}"
        ...
        for i, call in enumerate(progress_calls):
            if i == 0:
                ...
                assert call['current'] == 0, f"First call should have current=0, got {call['current']}"
            else:
                # Subsequent calls should be 1,2,3,4,5
                assert call['current'] == i, f"Progress current should be {i}, got {call['current']}"
            assert call['total'] == 5, f"Progress total should be 5, got {call['total']}"
