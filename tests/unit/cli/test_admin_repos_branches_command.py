"""
Unit tests for the 'cidx admin repos branches' command.
"""
...
class TestAdminReposBranchesCommand:
    ...
    def test_admin_repos_branches_command_exists(self, mock_detect_mode, runner):
    ...
    @patch("code_indexer.disabled_commands.detect_current_mode")
    def test_admin_repos_branches_requires_alias(self, mock_detect_mode, runner):
        """Test that the branches command requires an alias argument."""
        mock_detect_mode.return_value = "remote"

        result = runner.invoke(cli, ["admin", "repos", "branches"])
        assert result.exit_code != 0
        assert "Missing argument" in result.output or "ALIAS" in result.output

    @patch("code_indexer.disabled_commands.detect_current_mode")
    def test_admin_repos_branches_has_detailed_flag(self, mock_detect_mode, runner):
        """Test that the branches command has a --detailed flag."""
        mock_detect_mode.return_value = "remote"

        result = runner.invoke(cli, ["admin", "repos", "branches", "--help"])
        assert result.exit_code == 0
        assert "--detailed" in result.output
        assert "Show detailed branch information" in result.output
