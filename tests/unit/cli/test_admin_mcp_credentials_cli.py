...
class TestAdminMCPCredentialsListCommand:
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_list_credentials_for_user_table_format(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
             patch("code_indexer.cli._load_admin_credentials", return_value=({"username": "admin", "password": "pass"}, "http://test")), \
             patch("code_indexer.api_clients.admin_client.AdminAPIClient") as mock_admin_client:

            # Configure mock to return credentials
            mock_client_instance = MagicMock()
            mock_client_instance.list_mcp_credentials = AsyncMock(return_value={
                "credentials": [
                    {
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "list", "--user", "test_user"])
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_list_credentials_for_user_json_format(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance.list_mcp_credentials = AsyncMock(return_value={"credentials": credentials_data})
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "list", "--user", "test_user", "--format", "json"])
    ...
    def test_list_credentials_user_not_found(self, mock_mode_detector):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance = MagicMock()
            mock_client_instance.list_mcp_credentials = AsyncMock(side_effect=Exception("User not found"))
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "list", "--user", "nonexistent"])

            assert result.exit_code != 0
            assert "User not found" in result.output or "not found" in result.output.lower()
...
class TestAdminMCPCredentialsCreateCommand:
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_create_credential_for_user_table_format(self, mock_mode_detector, user_manager):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance = MagicMock()
            mock_client_instance.create_mcp_credential = AsyncMock(return_value={
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "create", "--user", "test_user", "--name", "New Credential"])
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_create_credential_for_user_json_format(self, mock_mode_detector, user_manager):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance.create_mcp_credential = AsyncMock(return_value=credential_data)
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "create", "--user", "test_user", "--name", "New Credential", "--format", "json"])
    ...
    def test_create_credential_without_name(self, mock_mode_detector, user_manager):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "create", "--user", "test_user"])
...
class TestAdminMCPCredentialsRevokeCommand:
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_revoke_credential(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance = MagicMock()
            mock_client_instance.revoke_mcp_credential = AsyncMock(return_value={"success": True})
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "revoke", "--user", "test_user", "--credential-id", credential_id])
    ...
    def test_revoke_credential_not_found(self, mock_mode_detector, user_manager):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance = MagicMock()
            mock_client_instance.revoke_mcp_credential = AsyncMock(side_effect=Exception("Credential not found"))
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "revoke", "--user", "test_user", "--credential-id", "nonexistent-id"])
...
class TestAdminMCPCredentialsListAllCommand:
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_list_all_credentials_table_format(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance = MagicMock()
            mock_client_instance.list_all_mcp_credentials = AsyncMock(return_value={
                "credentials": [
                    {
                        "username": "test_user",
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_list_all_credentials_json_format(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            mock_client_instance.list_all_mcp_credentials = AsyncMock(return_value={"credentials": credentials_data})
            mock_client_instance.close = AsyncMock()
            mock_admin_client.return_value = mock_client_instance

            result = runner.invoke(cli, ["admin", "mcp-credentials", "list-all", "--format", "json"])
    ...
    @patch("code_indexer.mode_detection.command_mode_detector.CommandModeDetector.detect_mode", return_value="remote")
    def test_list_all_credentials_with_limit(self, mock_mode_detector, user_manager, populated_credentials):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "list-all", "--limit", "1"])
