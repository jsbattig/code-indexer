...
class TestAdminMCPCredentialsEventLoopHandling:
    ...
    def test_list_credentials_no_event_loop_error(self, mock_mode_detector):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            assert "Event loop is closed" not in result.output, f"Found 'Event loop is closed' error in output:\n{result.output}"
    ...
    def test_create_credential_no_event_loop_error(self, mock_mode_detector):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "create", "--user", "test_user", "--name", "New Cred"])
            ...
            assert "Created" in result.output or "mcp_1234567890abcdef1234567890abcdef" in result.output
    ...
    def test_revoke_credential_no_event_loop_error(self, mock_mode_detector):
        ...
        with patch("code_indexer.mode_detection.command_mode_detector.find_project_root", return_value=Path.cwd()), \
            ...
            result = runner.invoke(cli, ["admin", "mcp-credentials", "revoke", "--user", "test_user", "--credential-id", "cred-123"])
