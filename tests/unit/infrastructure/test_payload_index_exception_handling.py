...
class TestPayloadIndexExceptionHandling(unittest.TestCase):
    ...
    def test_list_payload_indexes_exception_fixed_behavior(self):
        ...
        with patch.object(self.client, 'get_collection_info', 
            ...
            self.assertIn("error", status, "Status should contain error field when exception occurs")
            self.assertFalse(status.get("healthy", True), "Status should be unhealthy when exception occurs")
            
            # Should not contain false positive missing indexes
            self.assertNotIn("missing_indexes", status, "Should not calculate missing indexes when error occurs")
            self.assertNotIn("total_indexes", status, "Should not calculate total indexes when error occurs")
    ...
    def test_ensure_payload_indexes_query_context_with_exception(self):
        ...
        with patch.object(self.client, 'get_collection_info', 
            ...
            result = self.client.ensure_payload_indexes(collection_name, context="query")
            
            # Result should be False due to error, not True with false missing indexes
            self.assertFalse(result, "Should return False when exception occurs, not hide error with false positives")
            ...
            error_messages = [call for call in printed_calls if "Failed to list indexes" in call]
            ...
            missing_messages = [call for call in printed_calls if "Missing payload indexes" in call]
    ...
    def test_collection_not_found_still_returns_empty_list(self):
        ...
        with patch.object(self.client, 'get_collection_info',
            ...
            self.assertEqual(len(existing_indexes), 0,
