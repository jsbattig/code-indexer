...
class TestHintGeneratorQueryFailures:
    ...
    def test_query_failure_qdrant_connection_suggests_grep(self):
        ...
        assert any('rg' in cmd for cmd in hint.suggested_commands) or \
               any('grep' in cmd for cmd in hint.suggested_commands)
        ...
        assert 'qdrant' in hint.explanation.lower() or 'service' in hint.explanation.lower()
    ...
    def test_query_failure_with_search_term_in_commands(self):
        ...
        assert any('your-search-term' in cmd or 'search' in cmd for cmd in grep_commands)

    def test_query_failure_suggests_status_check(self):
        """Test query failure suggests checking cidx status."""
        hint = self.generator.generate_hint(
            command='query',
            error_text='Qdrant not running',
    ...
    def test_query_failure_generic_error(self):
        ...
        hint = self.generator.generate_hint(
            command='query',
            error_text='Unknown query error',
...
class TestHintGeneratorContainerFailures:
    ...
    def test_start_failure_port_conflict(self):
        ...
        assert any('docker ps' in cmd or 'podman ps' in cmd
    ...
    def test_start_failure_docker_unavailable(self):
        ...
        assert any('systemctl status docker' in cmd or 'systemctl status podman' in cmd
                  for cmd in hint.suggested_commands)

        # Should mention container runtime
        assert 'docker' in hint.message.lower() or 'podman' in hint.message.lower() or \
               'container' in hint.message.lower()
...
class TestErrorCategoryDetector:
    ...
    def test_detect_connection_error(self):
        ...
        assert self.detector.detect_category('Cannot connect to service') == 'connection'
    ...
    def test_detect_port_conflict(self):
        ...
        assert self.detector.detect_category('Port 6333 already in use') == 'port_conflict'
        assert self.detector.detect_category('Address already in use') == 'port_conflict'
        assert self.detector.detect_category('Bind failed on port 8080') == 'port_conflict'
    ...
    def test_detect_permission_error(self):
        ...
        assert self.detector.detect_category('Access denied to resource') == 'permission'
    ...
    def test_detect_configuration_error(self):
        ...
        assert self.detector.detect_category('Missing config parameter') == 'configuration'
...
class TestConversationRequirements:
    ...
    def test_query_failure_explains_why_alternative_needed(self):
        ...
        assert any(word in hint.explanation.lower()
