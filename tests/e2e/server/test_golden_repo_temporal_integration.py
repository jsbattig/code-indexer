...
class TestGoldenRepoTemporalIntegration:
    ...
    def test_execute_post_clone_workflow_with_temporal_parameters(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            subprocess.run(["git", "add", "."], cwd=str(repo_path), check=True, capture_output=True)
            subprocess.run(["git", "commit", "-m", "Second commit"], cwd=str(repo_path), check=True, capture_output=True)
            ...
            temporal_index_path = repo_path / ".code-indexer" / "index" / "code-indexer-temporal"
