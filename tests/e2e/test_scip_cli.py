...
""")
    ...
    assert rebuild_result.returncode != 0
    assert "Unknown project" in rebuild_result.stdout or "Unknown project" in rebuild_result.stderr
...
def test_scip_rebuild_validation_errors(tmp_path, monkeypatch):
    ...
    assert result.returncode != 0
    assert "No SCIP generation status found" in result.stdout or "scip generate" in result.stdout
