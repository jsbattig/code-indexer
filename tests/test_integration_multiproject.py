...
class TestMultiProjectIntegration(unittest.TestCase):
    ...
    def test_docker_compose_generation(self):
        ...
        self.assertEqual(
            services['ollama']['container_name'], 
            "code-ollama-test-project-1"
        )
        self.assertEqual(
            services['qdrant']['container_name'], 
            "code-qdrant-test-project-1"
        )
    ...
    def test_cli_integration_multiple_projects(self):
        ...
        ], capture_output=True, text=True, cwd=self.project1_path)
        
        # Test project 2 setup
        os.chdir(self.project2_path)
        result2 = subprocess.run([
            sys.executable, "-m", "code_indexer.cli", "status"
        ], capture_output=True, text=True, cwd=self.project2_path)
    ...
    def test_config_isolation(self):
        ...
        self.assertEqual(config1['model']['name'], config2['model']['name'])
