...
@pytest.fixture
def schema_migration_test_repo():
    """Create a test repository for schema migration E2E tests."""
...
def create_schema_migration_config(test_dir):
    """Create configuration for schema migration test."""
...
@pytest.fixture
def test_config(schema_migration_test_repo):
    """Create test configuration for schema migration tests."""
    test_dir = schema_migration_test_repo

    # Create configuration
    create_schema_migration_config(test_dir)
    ...
    if init_result.returncode != 0:
        pytest.skip(f"Service verification failed during init: {init_result.stderr}")
...
@pytest.fixture
def embedding_provider(test_config):
    """Create embedding provider for testing."""
    return EmbeddingProviderFactory.create(test_config)


@pytest.fixture
def collection_name(test_config, embedding_provider, qdrant_client):
...
def _create_legacy_points(
    qdrant_client: QdrantClient, embedding_provider, collection_name: str
) -> int:
...
def test_schema_version_detection(qdrant_client, embedding_provider, collection_name):
    ...
    legacy_count = _create_legacy_points(
...
def test_migration_safety_check(qdrant_client, embedding_provider, collection_name):
    """Test migration safety checks."""

    # Create legacy points
    _create_legacy_points(qdrant_client, embedding_provider, collection_name)
...
def test_manual_migration_execution(qdrant_client, embedding_provider, collection_name):
    ...
    assert (
        final_count >= result.content_points_created + result.visibility_points_created
    )
...
def test_migrated_data_functionality(
    ...
    for result in master_results:
        ...
        assert payload.get("type") == "content", "Search should return content points"
        assert "branch" not in payload, "Content points should not have branch field"
        assert "git_commit" in payload, "Content points should have git_commit field"
...
def test_automatic_migration_with_smart_indexer(
    test_config, schema_migration_test_repo
):
    ...
    metadata_path = schema_migration_test_repo / "metadata.json"
    ...
    assert stats.files_processed >= 2  # Should process at least README.md and main.py

    # COMPREHENSIVE VERIFICATION: Test search functionality
    print("🔍 Schema migration test: Verifying search functionality after migration...")
...
def test_mixed_schema_handling(qdrant_client, embedding_provider, collection_name):
    ...
    if (
        ...
        and visibility_point_retrieved.get("payload", {}).get("type") == "visibility"
    ):
