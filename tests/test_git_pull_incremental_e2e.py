...
class TestGitPullIncrementalE2E:
    ...
    def temp_project_dir(self):
        ...
        temp_dir = create_isolated_project_dir("git_pull_incremental")

        # Register the test in inventory
        create_test_project_with_inventory(
            temp_dir, TestProjectInventory.GIT_PULL_INCREMENTAL
        )
    ...
    def test_git_pull_with_deletions_no_reconcile_needed(self, temp_project_dir):
        """
        ...
        # Test environment setup handled in fixture

        # Set up git repo with remote
        remote_dir = self.setup_git_repo_with_remote(temp_dir)

        # Initialize code-indexer and do initial index
        ...
        assert "initial.py" not in result.stdout or "No results found" in result.stdout
    ...
    def test_multiple_git_pulls_with_complex_changes(self, temp_project_dir):
        ...
        subprocess.run(["code-indexer", "start", "--quiet"], cwd=temp_dir, check=True)
        ...
        assert "module_b.py" not in result.stdout or "No results found" in result.stdout
    ...
    def test_git_pull_with_working_directory_changes(self, temp_project_dir):
        ...
        assert "README.md" not in result.stdout or "No results found" in result.stdout
    ...
    def test_pressure_test_commit_watermark_tracking(self, temp_project_dir):
