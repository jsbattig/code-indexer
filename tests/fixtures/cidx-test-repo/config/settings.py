...
class ConfigLoader:
    ...
    def load_configuration(self, config_path: Optional[Union[str, Path]] = None) -> AppConfig:
        """
    ...
    def _create_config_object(self, config_data: Dict[str, Any]) -> AppConfig:
        ...
        main_fields = ["app_name", "app_version", "environment", "debug_mode", 
                      "data_dir", "temp_dir", "upload_max_size"]
        
        for config_field in main_fields:
            if config_field in config_data:
                if config_field == "environment":
                    setattr(app_config, config_field, Environment(config_data[config_field]))
                else:
    ...
    def _update_dataclass(self, obj: Any, data: Dict[str, Any]) -> None:
        ...
        for key, value in data.items():
            if hasattr(obj, key):
                ...
                # Handle enum fields
                if hasattr(current_value, '__class__') and issubclass(current_value.__class__, Enum):
...
def create_production_config() -> AppConfig:
    ...
    config = AppConfig(
        ...
        logging=LoggingConfig(
            level=LogLevel.INFO,
            file_path="/var/log/cidx/app.log",
