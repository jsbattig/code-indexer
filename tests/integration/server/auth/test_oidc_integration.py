...
class TestOIDCIntegration:
    ...
    @pytest.mark.asyncio
    async def test_full_oidc_flow_with_jit_provisioning(
        self, mock_oidc_server, oidc_manager
    ):
        ...
        mock_oidc_server.set_userinfo(
            sub="new-user-12345", email="newuser@example.com", email_verified=True
        )
    ...
    async def test_returning_oidc_user(self, mock_oidc_server, oidc_manager):
        ...
        mock_oidc_server.set_userinfo(
            sub="returning-user-789", email="returning@example.com", email_verified=True
        )
    ...
    async def test_oidc_discovery_endpoint(self, mock_oidc_server, oidc_manager):
        ...
        assert (
            metadata.authorization_endpoint == f"{mock_oidc_server.base_url}/authorize"
        )
    ...
    @pytest.mark.asyncio
    async def test_jwt_session_creation_after_oidc_login(
        self, mock_oidc_server, oidc_manager
    ):
        """Test that JWT session is created correctly after OIDC login."""
        # Setup user
        mock_oidc_server.set_userinfo(
            sub="session-test-user", email="session@example.com", email_verified=True
        )
    ...
    @pytest.mark.asyncio
    async def test_jit_provisioning_disabled(
        self, mock_oidc_server, test_users_file, tmp_path
    ):
        ...
        mock_oidc_server.set_userinfo(
            sub="new-user-no-jit", email="nojit@example.com", email_verified=True
        )
    ...
    @pytest.mark.asyncio
    async def test_email_verification_required_rejects_unverified(
        self, mock_oidc_server, test_users_file, tmp_path
    ):
    ...
    @pytest.mark.asyncio
    async def test_email_verification_not_required_allows_unverified(
        self, mock_oidc_server, test_users_file, tmp_path
    ):
