...
"""
    ...
    def _connect_to_daemon(self):
        ...
        return rpyc.connect(str(self.socket_path), config={"allow_all_attrs": True})
    ...
    def test_cache_hit_performance_real_data(self):
        ...
        try:
            ...
            for i in range(50):
                start = time.perf_counter()
                conn.root.query(str(self.project_path), f"test query {i}", limit=5)
    ...
    def test_daemon_shutdown_removes_socket(self):
        ...
        if self.daemon_process:
            ...
            self.assertIsNotNone(
                self.daemon_process.poll(), "Daemon process should have terminated"
            )

        # Verify socket removed
        self.assertFalse(
            self.socket_path.exists(), "Socket file should be removed after shutdown"
        )
        ...
        try:
            ...
            def run_queries(client_idx):
                ...
                for i in range(10):
                    start = time.perf_counter()
                    conn.root.query(
                        str(self.project_path), f"query {client_idx}-{i}", limit=5
                    )
                    times.append(time.perf_counter() - start)
                return times
            ...
            with ThreadPoolExecutor(max_workers=5) as executor:
                futures = [executor.submit(run_queries, i) for i in range(5)]
                all_times = []
...
"""
    ...
    def test_storage_operations_cache_coherence(self):
        ...
        try:
            ...
            with patch(
                "src.code_indexer.services.cleanup_service.CleanupService.clean_vectors"
            ):
            ...
            with patch(
                "src.code_indexer.services.cleanup_service.CleanupService.clean_data"
            ):
