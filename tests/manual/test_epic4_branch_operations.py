...
class Epic4BranchTester:
    ...
    def setup_test_repository(self):
        ...
        self.test_repo_path = Path(
            "/home/jsbattig/Dev/code-indexer/test-data/epic4-branch-test"
        )
        ...
        try:
            ...
            subprocess.run(
                ["git", "commit", "-m", "Initial commit on main"], check=True
            )
            ...
            subprocess.run(["git", "add", "."], check=True)
            subprocess.run(
                ["git", "commit", "-m", "Add feature implementation"], check=True
            )
    ...
    def register_golden_repository(self, alias, branch="master"):
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                for i in range(30):  # Wait up to 30 seconds
                    ...
                    if job_response.status_code == 200:
                        ...
                        if job_data["status"] == "completed":
                            print(
                                f"✅ Golden repository '{alias}' registration completed"
                            )
                            return True
                        elif job_data["status"] == "failed":
                            print(
                                f"❌ Golden repository registration failed: {job_data.get('error', 'Unknown error')}"
                            )
                            return False
                ...
                print(
                    f"❌ Golden repository registration failed: {response.status_code} - {response.text}"
                )
                return False
    ...
    def activate_repository(self, golden_alias, user_alias, branch=None):
        ...
        activate_data = {"golden_repo_alias": golden_alias, "user_alias": user_alias}
        if branch:
        ...
        try:
            ...
            if response.status_code == 202:
                ...
                for i in range(60):  # Wait up to 60 seconds
                    ...
                    if job_response.status_code == 200:
                        ...
                        if job_data["status"] == "completed":
                            ...
                            print(
                                f"❌ Repository activation failed: {job_data.get('error', 'Unknown error')}"
                            )
                            return False
                ...
                print(
                    f"❌ Repository activation failed: {response.status_code} - {response.text}"
                )
                return False
    ...
    def switch_branch(self, user_alias, branch):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Branch switch failed: {response.status_code} - {response.text}"
                )
                return False
    ...
    def query_repository(self, user_alias, query_text):
        ...
        query_data = {"query": query_text, "repository_alias": user_alias, "limit": 5}
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for i, res in enumerate(result["results"][:3]):  # Show first 3 results
                    print(
                        f"   {i+1}. {res['file_path']}:{res['line_number']} (score: {res['similarity_score']:.3f})"
                    )
    ...
    def test_story_4_1_branch_switching(self):
        ...
        if results["4.1.1"]:
            ...
            print("\n🧪 Test 4.1.2: Branch switch updates indexing data")
            query_result = self.query_repository(
                "test-repo-main", "feature implementation"
            )
            results["4.1.2"] = (
                query_result is not None and len(query_result["results"]) > 0
            )
        ...
        try:
            ...
            if results["4.1.4"]:
                print(
                    f"✅ Non-existent branch properly rejected ({response.status_code})"
                )
            else:
    ...
    def test_story_4_2_branch_activation_variations(self):
        ...
        results["4.2.1"] = self.activate_repository(
            "epic4-test", "test-main-branch", "master"
        )
        ...
        results["4.2.2"] = self.activate_repository(
            "epic4-test", "test-master-branch", "master"
        )

        # Test 4.2.3: Activate repository on feature/branch-test branch
        print("\n🧪 Test 4.2.3: Activate repository on feature/branch-test branch")
        results["4.2.3"] = self.activate_repository(
            "epic4-test", "test-feature-branch", "feature/branch-test"
        )

        # Test 4.2.4: Activate repository on hotfix/bug-fix branch
        print("\n🧪 Test 4.2.4: Activate repository on hotfix/bug-fix branch")
        results["4.2.4"] = self.activate_repository(
            "epic4-test", "test-hotfix-branch", "hotfix/bug-fix"
        )
        ...
        if results["4.2.3"]:  # feature branch
            feature_results = self.query_repository(
                "test-feature-branch", "new feature"
            )
    ...
    def test_story_4_3_branch_content_verification(self):
        ...
        hotfix_specific = self.query_repository(
            "test-hotfix-branch", "fix_critical_bug"
        )
        ...
        main_found = main_main_py and any(
            "main_branch" in r["code_snippet"] for r in main_main_py["results"]
        )
        feature_found = feature_main_py and any(
            "feature_branch" in r["code_snippet"] for r in feature_main_py["results"]
        )
