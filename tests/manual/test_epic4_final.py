...
def test_epic4_branch_operations():
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            results["branch_switch_endpoint"] = "partial"
            print(f"⚠️  Branch switch endpoint responded unexpectedly ({response.status_code})")
    ...
    epic_tests = {
        "4.1.1 - Branch switch API exists": results.get("branch_switch_endpoint", False),
        ...
        "4.3.1 - Repository functional after operations": results.get("post_branch_query", False),
    }
    ...
    if results.get("branch_switch_endpoint") == True:
        ...
        print("   - This could be due to local repository limitations (no remote origin)")
    else:
