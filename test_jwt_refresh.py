...
def test_jwt_refresh():
    ...
    try:
        ...
        print(f"   User: {refresh_result['user']['username']} ({refresh_result['user']['role']})")
    ...
    try:
        ...
        if repos_response.status_code == 200:
            ...
            print(f"⚠️  Protected endpoint returned: {repos_response.status_code} - {repos_response.text}")
    ...
    try:
        ...
        if invalid_refresh_response.status_code == 401:
            ...
            print(f"⚠️  Expected 401 for invalid token, got: {invalid_refresh_response.status_code}")
